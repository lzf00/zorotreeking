...
test("G317 Chengdu and Lhasa-return loop are independent indexed roadbooks", async () => {
  ...
  assert.match(loopGuide, /10 月 4 日住班戈/);
  assert.match(loopGuide, /纳木错富氧改成班戈尚客优品/);
...
test("Lhasa-return loop covers Sep 26 through Oct 7 without early-flight friend exits", () => {
  ...
  assert.match(aliLhasaReturnRouteDays[3]?.title ?? "", /定日.*珠峰景区.*巴松顺康/);
  assert.match(aliLhasaReturnRouteDays[4]?.title ?? "", /巴松顺康.*萨嘎/);
...
test("Lhasa-return one-page sheet lists sights, distance, and hotels for every day", async () => {
  ...
  assert.match(aliLhasaReturnDailyPlanning[8]?.stay.hotels[0]?.name ?? "", /班戈/);
  assert.doesNotMatch(aliLhasaReturnDailyPlanning[8]?.stay.hotels[0]?.name ?? "", /纳木错富氧/);
