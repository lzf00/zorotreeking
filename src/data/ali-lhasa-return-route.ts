...
const friendExitPoints: AliRoutePoint[] = [
  {
    ...
    lng: 80.0563,
    elevationM: 4274,
    detail: "朋友提前返沪的首选分流机场，距狮泉河镇规划道路约 54 公里。团队 10 月 2 日抵达狮泉河后，朋友可于 10 月 3 日或 4 日单独前往机场。",
...
export const aliLhasaReturnRoutePoints: AliRoutePoint[] = [
  ...aliRoutePoints.map((point) => {
    ...
    if (point.id === "shiquanhe") {
      return {
        ...point,
        detail: "阿里地区综合补给中心，也是朋友最晚 10 月 5 日抵沪的最佳分流城市；本版 10 月 2 日到达狮泉河，朋友 10 月 3 日或 4 日从昆莎机场离开。",
      };
    }
    if (point.id === "dongco") {
      return {
        ...
        detail: "G317 快速返拉版的关键乡镇落脚点，用来把狮泉河到班戈的两段长路拆开；住宿条件必须提前电话确认。",
...
replaceCopiedDrivingDay("09.29", {
  title: "定日 → 加乌拉 → 珠峰景区开放区域",
  distance: "约 110–150 km",
  driving: "约 4–6 小时，上午进景区，傍晚留在景区看日落",
  roads: "G318 / 珠峰景区道路",
  pointIds: ["tingri", "gawula", "rongbuk", "ebc"],
  highlights: ["加乌拉群峰", "绒布寺", "珠峰日落", "珠峰星空"],
  supply: "定日早餐后满油出发，午餐和热水随车带入；景区住宿点只作为过夜和保暖恢复，不依赖复杂补给。",
  overnight: "珠峰景区开放住宿点，优先绒布寺附近或景区统一开放住宿；若景区住宿未确认，则住巴松/定日并放弃星空。",
  risk: "本晚海拔高、风冷强，不能把看完日落再赶去萨嘎当作计划；星空拍摄只在住宿点附近活动。",
  decision: "只有门票、观光车和景区住宿三项都确认，才执行景区过夜；上午无法进景区时，不晚进晚出，不夜间翻回定日。",
});

replaceCopiedDrivingDay("09.30", {
  title: "珠峰景区 → 定日 → 萨嘎",
  distance: "约 330–380 km",
  driving: "约 8–10 小时，早上离开珠峰景区",
  roads: "珠峰景区道路 / G318 / G219",
  pointIds: ["ebc", "rongbuk", "gawula", "tingri", "saga"],
  highlights: ["珠峰晨光", "加乌拉回望", "G219 西行", "萨嘎补给"],
  supply: "从景区带早餐离开，回到定日/白坝片区补油和热食，再沿 G219 去萨嘎；抵达后补满燃油。",
  overnight: "萨嘎县城。",
  risk: "前一晚高海拔睡眠质量可能差，本日不再安排新的深度景点；若早晨风雪或观光车延误，优先保证白天抵达萨嘎。",
  decision: "10:00 前离开珠峰方向；若中午仍未回到定日主路，取消沿途停留，只把萨嘎作为安全住宿目标。",
});
...
replaceCopiedDrivingDay("10.05", {
  title: "班戈 → 纳木措 → 当雄 → 拉萨",
  distance: "约 550–600 km",
  driving: "约 10–12 小时，晚间或深夜到店",
  ...
  decision: "07:30 前从班戈出发；若纳木措关闭、摆渡排队或午后仍未完成湖区通过，直接经当雄回拉萨，守住当晚住宿。",
});
...
export const aliLhasaReturnDailyPlanning: AliRouteDailyPlanning[] = [
  ...
  },
  {
    ...
    reservations: [
      ...
      },
      {
        ...
        status: "建议提前",
        leadTime: "提前 1–3 天锁定门票、观光车和景区内可住床位，抵达定日后再次核验班次",
        channel: "正规在线旅游平台、景区售票渠道和景区住宿点；不向个人转账买票或订房",
        documents: "二代身份证、电子边境通行证；优惠人群带证件原件；住宿确认记录",
        note: "本版按上午进珠峰景区、当晚留在景区看日落和星空处理。购票后保存订单二维码，前一晚核对发车时间、风雪、开放区域和夜间住宿保留规则。",
  ...
  },
  {
    ...
    reservations: [
      {
        ...
        status: "必须提前",
        leadTime: "前一晚完成订单、证件、首班观光车和景区住宿复核；建议提前 1–3 天购票订房",
        channel: "景区正规票务/在线旅游平台 + 定日游客换乘点 + 景区开放住宿点",
        documents: "二代身份证、电子边境通行证、门票与观光车订单、住宿确认记录",
        note: "社会车辆按现场组织停放并换乘观光车；上午进景区，傍晚不离开珠峰区域，日落和星空只在住宿点附近看。若景区住宿或夜间开放规则无法确认，改住定日或巴松，取消星空。",
    ...
    stay: {
      city: "珠峰景区开放住宿点",
      budget: standardRoomBudget,
      bookingAdvice: `${nationalDayHotelAdvice} 本晚是全程最关键的高海拔住宿，订房优先级高于拍摄计划；必须确认供暖、供氧或应急氧、晚餐热水和夜间保留床位。`,
      hotels: [
        {
          name: "珠峰大本营开放住宿点",
          strengths: "位置最适合日落和星空，但条件简陋、海拔高；必须提前确认当季是否开放、保暖和应急氧。",
          bookingUrl: "https://m.ctrip.com/webapp/hotel/undefined201",
        },
        {
          name: "绒布寺 / 巴松片区住宿备选",
          strengths: "比大本营住宿点更适合作为天气或满房时的退路；订前确认距景区换乘点、停车和夜间热水。",
          bookingUrl: "https://m.ctrip.com/webapp/hotel/tingri201",
        },
      ],
    },
  },
    ...
    reservations: [
      {
        ...
        status: "出发前复核",
        leadTime: "9 月 29 日入住时确认次日清晨离场、摆渡、道路和萨嘎住宿",
        channel: "景区住宿点、定日游客换乘点、当地交警和萨嘎酒店",
        documents: "身份证、电子边境通行证、门票订单、萨嘎酒店订单",
        note: "本日只做珠峰晨光后离场，不再把佩枯措或其他支线设为必到点。前一晚睡眠差、头痛明显或风雪延误时，优先安全抵达萨嘎。",
        sourceLabel: "西藏文旅厅 · 珠峰景区 2026 实名与观光车",
        sourceUrl: "https://wlt.xizang.gov.cn/xccx/lytg/202603/t20260316_529530.html",
      },
    ],
  ...
  },
  {
    ...
    reservations: [
      ...
      },
      {
        ...
        note: "本版把珠峰景区住宿放回行程，札达仍只做穿行远观或短停。未查到 2026 国庆统一强制预约条款；按正规开放道路进入，不驶入未开放土林沟谷。",
      ...
      },
      {
        ...
        note: "前景优先版把时间给珠峰景区夜宿和神山圣湖，不再同时安排古格和托林寺。若排队、天气或道路耗时超预期，直接取消参观并前往狮泉河。",
    ...
    stay: {
      city: "狮泉河镇",
      budget: standardRoomBudget,
      bookingAdvice: `${nationalDayHotelAdvice} 本版 10 月 2 日住狮泉河，前半程换回珠峰景区住宿；到店后必须复核洞措住宿和 10 月 5 日拉萨保留房。`,
      hotels: [
        {
    ...
    reservations: [
      {
        subject: "朋友昆莎机场返沪机票",
        status: "必须提前",
        leadTime: "建议提前 30–60 天出票，优先 10 月 3 日离开，最晚 10 月 4 日离开",
        channel: "航空公司官方渠道；机场接送需另行与正规车辆书面确认",
        documents: "身份证、航班订单、行李额度与接送订单",
        note: "只适用于 10 月 5 日必须抵沪的朋友：10 月 2 日晚在狮泉河与团队分流，不再随车进入 G317。10 月 4 日离开已经没有航变余量。",
      ...
      },
      {
        ...
        note: "狮泉河到拉萨段按快返处理，沿途不新增景点。革吉、雄巴短停，改则强制补油和热食；当天真正需要提前锁定的是洞措住宿、车辆救援联系方式和次日班戈房间。",
  ...
  },
  {
    ...
    reservations: [
      {
        ...
        note: "10 月 5 日把纳木措作为狮泉河到拉萨段第二个大景，但只在开放道路和正规观景点短停；不安排圣象天门、扎西半岛深度游或湖边支线，当雄只做补给与状态检查。",
    ...
    stay: {
      city: "拉萨市区",
      budget: standardRoomBudget,
      bookingAdvice: `${nationalDayHotelAdvice} 本晚从班戈经纳木措返拉，可能晚间或深夜到店；必须预付保留房并电话确认 24 小时前台，优先与 10 月 6 日连住。`,
      hotels: [
        {
          name: "如家商旅酒店（拉萨堆龙经济开发区店）",
