...
add_header Content-Security-Policy "
  default-src 'self';
  script-src 'self' 'unsafe-inline' 'unsafe-eval' https://unpkg.com https://cdn.jsdelivr.net https://giscus.app https://webapi.amap.com;
...
4. **高德地图**：`script-src` 只放行官方 SDK 主机；地图运行时请求只放行高德与 Autonavi 子域。部署流水线会幂等核对这三项并在 `nginx -t` 失败时恢复备份。
